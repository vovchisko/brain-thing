...
const bus = createBus('attributes')
...
export async function handle ({ tags, project } = {}) {
  ...
  const typeDefs = cfg.state.vault.attributes || {}
  let response = `Attributes across ${ entries.length } entries`
  ...
  for (const [ attribute, stat ] of sorted) {
    const def = typeDefs[attribute]
    const type = def ? def.type : 'string'
    const pct = Math.round(stat.count / entries.length * 100)

    response += `\n- **${ attribute }** (${ type }) — ${ stat.count } entries (${ pct }%)`
