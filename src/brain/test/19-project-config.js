...
export default async function ({ post, get, assert }) {
  ...
  const list = await post(TOOLS.PROJECT_CONFIG, {})
  assert(list.data.text.includes('TestProject'), 'lists TestProject')

  // Create new project
  const create = await post(TOOLS.PROJECT_CONFIG, {
    project: 'NP',
    folder: 'New Project',
  ...
  const home = await post(TOOLS.GET, { name: 'NP' })
  ...
  const update = await post(TOOLS.PROJECT_CONFIG, {
    project: 'NP',
  ...
  // Remove project
  const remove = await post(TOOLS.PROJECT_CONFIG, { project: 'NP', remove: true })
  ...
  const stillThere = await post(TOOLS.GET, { name: 'NP' })
  assert(stillThere.data.text.includes('NP'), 'entry still exists after project removal')

  // Remove nonexistent
  const noSuch = await post(TOOLS.PROJECT_CONFIG, { project: 'NOPE', remove: true })
  ...
  const create2 = await post(TOOLS.PROJECT_CONFIG, { project: 'NP', folder: 'NP Reborn' })
  ...
  await post(TOOLS.PROJECT_CONFIG, { project: 'ZZ', folder: 'Zzz' })
  // TestProject must still exist with its rules
  const list3 = await post(TOOLS.PROJECT_CONFIG, { project: 'TestProject' })
  ...
  const badFolder = await post(TOOLS.PROJECT_CONFIG, { project: 'VV', folder: 'My:Project' })
  assert(badFolder.data.text.includes('invalid characters'), 'rejects bad folder chars')

  const badRuleFolder = await post(TOOLS.PROJECT_CONFIG, {
  ...
  const dotFolder = await post(TOOLS.PROJECT_CONFIG, { project: 'VV', folder: '.hidden' })
  assert(dotFolder.data.text.includes('dot'), 'rejects dot folder')

  // Validation: bad project key
  const badKey = await post(TOOLS.PROJECT_CONFIG, { project: 'A/B', folder: 'Test' })
  assert(badKey.data.text.includes('invalid'), 'rejects bad project key')

  // Cleanup
  await post(TOOLS.PROJECT_CONFIG, { project: 'NP', remove: true })
  await post(TOOLS.PROJECT_CONFIG, { project: 'ZZ', remove: true })
}
